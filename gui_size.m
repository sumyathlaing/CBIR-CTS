function varargout = gui_size(varargin)
% GUI_SIZE MATLAB code for gui_size.fig
%      GUI_SIZE, by itself, creates a new GUI_SIZE or raises the existing
%      singleton*.
%
%      H = GUI_SIZE returns the handle to a new GUI_SIZE or the handle to
%      the existing singleton*.
%
%      GUI_SIZE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_SIZE.M with the given input arguments.
%
%      GUI_SIZE('Property','Value',...) creates a new GUI_SIZE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI_SIZE before gui_size_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to gui_size_OpeningFcn via varargin.
%
%      *See GUI_SIZE Options on GUIDE's Tools menu.  Choose "GUI_SIZE allows only one
%      instance to run (singleton)".
%
% See also: GUIDE, GUIDATA, GUIHANDLES

% Edit the above text to modify the response to help gui_size

% Last Modified by GUIDE v2.5 25-Oct-2015 22:59:53

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_size_OpeningFcn', @gui_size_OpeningFcn, ...
    'gui_size_OutputFcn',  @gui_size_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before gui_size is made visible.
function gui_size_OpeningFcn(hObject, eventdata, handles, varargin)
    clc;
    %Results tab
 ...
 %%                   'XTick',[],'YTick',[],...
   %%                 'Color',handles.unselectedTabColor,...
     %%               'Position',[pos2(1) pos2(2) pos2(3)+0.05 pos2(4)],...
       %%             'ButtonDownFcn','gui_size(''a2bd'',gcbo,[],guidata(gcbo))');
%%%    handles.t2=text('String','Intermediate Results','Units','normalized',...
         ...
         %%           'Margin',0.001,...
           %%         'FontSize',8,...
             %%       'Backgroundcolor',handles.unselectedTabColor,...
               %%     'ButtonDownFcn','gui_size(''t2bd'',gcbo,[],guidata(gcbo))');
  ...
  %%  set(handles.intermResults,'Visible','off')
    ...
    % Choose default command line output for gui_size
    handles.output = hObject;

    % Update handles structure
    guidata(hObject, handles);

    %Display gui_size in center of screen
...
function varargout = gui_size_OutputFcn(hObject, eventdata, handles)
...
function [query fusion bag_of_word]=build_options(handles)
   ...
   %2 end
    %2if get(handles.rdGlobalTexture,'Value')%use global texture features
     %2   if get(handles.rdEHD, 'Value')
       %2     texture='EHD_global';
         %2   textureFeat='Query_Features.EHD_global=ehd(query_image,0.1)';
        %2 end
 ...
 %       load('test_local.mat');
  %    else
   ...
   % end
    ...
    %2 if get(handles.chboxColorSketch, 'Value')
     %2   feat=fusion{1};
      %2  evalc(query{1});
...
%         [grid] = bestColMatch(gridcolors, colSpace, contents{get(handles.dropdownNumberOfBins,'Value')});
    if get(handles.chboxColorFeatures, 'Value') && get(handles.chboxTextureFeatures, 'Value')
        do_fusion=1;

        if get(handles.rdnofusion,'Value')
          %2  if bag_of_word.both
          %2      evalc(query{3});
          %2      feat=fusion{3};
          %2      do_fusion=0;
          %2      distfun=distfuncolor;
