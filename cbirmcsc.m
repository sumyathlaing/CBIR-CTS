function varargout = cbirmcsc(varargin)
% CBIRMCSC MATLAB code for cbirmcsc.fig
%      CBIRMCSC, by itself, creates a new CBIRMCSC or raises the existing
%      singleton*.
%
%      H = CBIRMCSC returns the handle to a new CBIRMCSC or the handle to
%      the existing singleton*.
%
%      CBIRMCSC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CBIRMCSC.M with the given input arguments.
%
%      CBIRMCSC('Property','Value',...) creates a new CBIRMCSC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the CBIRMCSC before cbirmcsc_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to cbirmcsc_OpeningFcn via varargin.
%
%      *See CBIRMCSC Options on GUIDE's Tools menu.  Choose "CBIRMCSC allows only one
%      instance to run (singleton)".
%
% See also: GUIDE, GUIDATA, GUIHANDLES

% Edit the above text to modify the response to help cbirmcsc

% Last Modified by GUIDE v2.5 04-Jun-2016 23:31:09

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'cbirmcsc_OpeningFcn', @cbirmcsc_OpeningFcn, ...
    'cbirmcsc_OutputFcn',  @cbirmcsc_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before cbirmcsc is made visible.
function cbirmcsc_OpeningFcn(hObject, eventdata, handles, varargin)
    clc;
    %Results tab
 ...
 %%                   'XTick',[],'YTick',[],...
   %%                 'Color',handles.unselectedTabColor,...
     %%               'Position',[pos2(1) pos2(2) pos2(3)+0.05 pos2(4)],...
       %%             'ButtonDownFcn','cbirmcsc(''a2bd'',gcbo,[],guidata(gcbo))');
%%%    handles.t2=text('String','Intermediate Results','Units','normalized',...
         ...
         %%           'Margin',0.001,...
           %%         'FontSize',8,...
             %%       'Backgroundcolor',handles.unselectedTabColor,...
               %%     'ButtonDownFcn','cbirmcsc(''t2bd'',gcbo,[],guidata(gcbo))');
  ...
  %%  set(handles.intermResults,'Visible','off')
    ...
    % Choose default command line output for cbirmcsc
    handles.output = hObject;

    % Update handles structure
    guidata(hObject, handles);

    %Display cbirmcsc in center of screen
...
function varargout = cbirmcsc_OutputFcn(hObject, eventdata, handles)
